...
const registerUser = async (req, res) => {
    try {
        ...
        await newUser.save();
        res.status(201).send({ success: true, message: 'Successfully created new user' })
    } catch (err) {
    ...
    try {
        ...
        // user exits in the database
            ...
            // password is correct
                ...
                res.cookie('accessToken', jwtToken, { httpOnly: true, sameSite: 'none', secure: true, expires: (new Date(Date.now() + 15 * 24 * 60 * 60 * 1000) )})
                    .status(200)
